...
class GraphLayer(nn.Module):
    def __init__(self, inp_dim, dim=64, k1=40, k2=20):
        super(GraphLayer, self).__init__()
        self.dim = dim
        self.inp_dim = inp_dim *2 + 9
        self.k1 = k1

        self.bn1 = nn.BatchNorm2d(self.dim)
        self.bn2 = nn.BatchNorm2d(self.dim)

        self.conv1 = nn.Sequential(nn.Conv2d(self.inp_dim, self.dim, kernel_size=1, bias=False),
                                   self.bn1,
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(self.inp_dim, self.dim, kernel_size=1, bias=False),
                                    self.bn3,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.conv4 = nn.Sequential(nn.Conv2d(self.dim, self.dim, kernel_size=1, bias=False),
                                    self.bn4,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv2d(self.inp_dim, self.dim, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        # self.bn1 = nn.BatchNorm2d(self.dim)
        # self.bn2 = nn.BatchNorm2d(self.dim)
    ...
    def forward(self, x, points):
        # pdb.set_trace()
        x_knn1 = get_graph_feature(x, points, k=self.k1)  # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
        ...
        # print("x_k1 shape:", x_k1.shape)
        # print("x_knn1 shape:", x_knn1.shape)
        return x_k1
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h, adj):        
        Wt = self.W.unsqueeze(0).repeat(h.shape[0],1,1)
        Wh = torch.bmm(h, Wt) 
...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, dropout, alpha, nheads, out_dim):
...
def entire_gat(nhid, nb_heads, features, out_dim):
    adj = np.zeros((features.shape[1], features.shape[1]))
    adj[:,0] = 1
    adj[0,:] = 1
    adj[0,0] =0 
    adj1 = sys_normalized_adjacency(adj)
    values = adj1.data
    ...
    adj1 = torch.sparse.FloatTensor(i, v, torch.Size(shape)).to_dense().cuda()
    # print("adj1 shape: ", adj1.shape)

    model = GAT(nfeat=features.shape[2], 
                nhid=nhid, # 8
                dropout=0.6, 
                nheads=nb_heads, # 8
                alpha=0.2,
...
class PointNetFeatures(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k1=40, k2=20):
        ...
        self.k1 = k1
        self.k2 = k2
        self.graph_layer = GraphLayer(dim=64, inp_dim=64, k1=self.k1,  k2=self.k2)
        self.graph_global = GraphLayer(dim=64, inp_dim=64 ,k1=self.num_points, k2=self.k2)
        self.sa = SelfAttentionLayer(64)
    ...
    def forward(self, x):
        n_pts = x.size()[2]
        # print("n_pts: ", n_pts)
        # print("_____________POINTFEATURE________________")
        ...
        # print("x in pointfeat:", x.shape)

        x_k = self.graph_layer(x, points)
        # print(x_k.shape)
        # x_global = self.graph_global(x, points)
        # x_global, x_w = self.sa(x_global.permute(0,2,1))
        x_global = entire_gat(8, 8, x.permute(0,2,1), 64)
        x_global = x_global.permute(0,2,1)
        # print(x_global.shape)
        x = self.ada_layer([x_global, x_k])
        # print(x.shape)
...
class PointNetEncoder(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k1=40, k2=20):
        ...
        self.graph_layer = GraphLayer(dim=128, inp_dim=128, k1=self.k1, k2=self.k2)
        self.graph_global = GraphLayer(dim=128, inp_dim=128 ,k1=self.num_points, k2=self.k2)
        self.sa = SelfAttentionLayer(128)
        self.ada_layer = AdaptiveLayer(C=128)


    def forward(self, points):
        # print("______________________pnt_______________________")
        # print("points shape pointnet:", points.shape)
        n_pts = points.size()[2]
        ...
        x_k = self.graph_layer(x1, points)
        # print("x_k shape:", x_k.shape)
        # x_global = self.graph_global(x1, points)
        # print("x_global shape:", x_global.shape)
        # x_global, x_w = self.sa(x_global.permute(0,2,1))
        x_global = entire_gat(8, 8, x1.permute(0,2,1), 128)
        x_global = x_global.permute(0,2,1)
        # print("x_global shape:", x_global.shape)
        # print("x_global shape:", x_global.shape)
        # print("x_k shape:", x_k.shape)
        x1 = self.ada_layer([x_global, x_k])
        # print("x1 shape:", x1.shape)

        x2 = F.relu(self.bn3(self.conv3(x1)))  # (128,256,128)
...
def get_graph_feature(x, points, k=20, idx=None, dim9=False):
    ...
    points = points.view(batch_size, -1, num_points)
    if idx is None:
        if dim9 == False:
