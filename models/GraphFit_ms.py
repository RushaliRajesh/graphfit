...
class PointNetEncoder(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k1=40, k2=20):
        ...
        self.conv1 = nn.Sequential(torch.nn.Conv1d(256,512,1), nn.BatchNorm1d(512), nn.LeakyReLU())
        self.conv2 = nn.Sequential(torch.nn.Conv1d(512,512,1), nn.BatchNorm1d(512), nn.LeakyReLU())

        self.conv_final = nn.Sequential(torch.nn.Conv1d(512+256,512,1), nn.BatchNorm1d(512), nn.LeakyReLU())

        # self.conv3 = nn.Sequential(torch.nn.Conv1d(256,256,1), nn.BatchNorm1d(256), nn.LeakyReLU())
        # self.conv4 = nn.Sequential(torch.nn.Conv1d(256,512,1), nn.BatchNorm1d(512), nn.LeakyReLU())
        ...
        # self.gap_block_2 = GAPBlock(self.n_heads, self.k1, in_dim=256, out_dim=256)
