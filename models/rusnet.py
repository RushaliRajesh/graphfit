...
class GAPLayer(nn.Module):
    def __init__(self, k):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1, bias=False), nn.BatchNorm2d(64), nn.LeakyReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=1, bias=False), nn.BatchNorm2d(1), nn.LeakyReLU())

        self.conv3 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=1, bias=False), nn.BatchNorm2d(64), nn.LeakyReLU())
        self.conv4 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=1, bias=False), nn.BatchNorm2d(1), nn.LeakyReLU())
    ...
    def forward(self, x):
        ...
        x_feat = torch.matmul(alpha.permute(0,1,3,2), xl)

        return x_feat, xl


class GAPBlock(nn.Module):
    def __init__(self, n_heads, k):
        ...
        for i in range(n_heads):
            self.gap_layers.append(GAPLayer(self.k))
...
class PointNetFeatures(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k1=40, k2=20):
        ...
        self.gap_block = GAPBlock(self.n_heads, self.k1)
