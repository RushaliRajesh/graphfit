...
class PointNetFeatures(nn.Module):
    ...
    def forward(self, x):
        ...
        # # print("x in pointfeat:", x.shape)
        # x = self.graph_global(x, points)
        # x_k = self.graph_layer(x, points)
        # # pdb.set_trace()
        # x, x_w = self.sa(x.permute(0,2,1))
        # x = x.permute(0,2,1)
...
class AdaptiveLayer(nn.Module):
    def __init__(self, C, r=8):
        ...
        self.ca = CrossAttentionLayer(C)
    def forward(self, x):
        fea_v, fea_w = self.ca(x[0].permute(0,2,1), x[1].permute(0,2,1))
        fea_v = fea_v.permute(0,2,1)
        # fea = x[0] + x[1]
        # b, C, _ = fea.shape
