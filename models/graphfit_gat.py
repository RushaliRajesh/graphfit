...
class GraphLayer(nn.Module):
    def __init__(self, dim=64, k1=40, k2=20):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(137, self.dim, kernel_size=1, bias=False),
                                   self.bn1,
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(137, self.dim, kernel_size=1, bias=False),
                                    self.bn3,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.conv4 = nn.Sequential(nn.Conv2d(self.dim, self.dim, kernel_size=1, bias=False),
                                    self.bn4,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv2d(137, self.dim, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))

    def forward(self, x, points):
        # pdb.set_trace()
        x_knn1 = get_graph_feature(x, points, k=self.k1) 
        x_knn1 = self.conv1(x_knn1)  
        x_knn1 = self.conv2(x_knn1) 
        x_k1 = x_knn1.max(dim=-1, keepdim=False)[0] 
        return x_k1
...
class PointNetFeatures(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, points], 1) #(128,67,128)
        x = entire_gat(8, 8, x.permute(0,2,1), 64)
        x = x.permute(0,2,1)
...
class PointNetEncoder(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1, sym_op='max', k1=40, k2=20):
        ...
        # self.graph_global = GraphLayer(dim=128, k1=self.num_points, k2=self.k2)
    ...
    def forward(self, points):
        ...
        x1 = entire_gat(8, 8, x1.permute(0,2,1), 128)
        x1 = x1.permute(0,2,1)
        # pdb.set_trace()
